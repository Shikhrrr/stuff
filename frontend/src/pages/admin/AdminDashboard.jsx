...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-[#1C1C1C]">Dashboard Overview</h1>
        <p className="text-[#6B6B6B] mt-1">Welcome back. Here's what's happening today.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {statCards.map((stat, idx) => {
          ...
          return (
            <div key={idx} className="bg-white p-6 rounded-3xl border border-[#F5C6D0]/30 shadow-sm flex items-center gap-4">
              <div className={`p-4 rounded-full bg-gray-50 ${stat.color}`}>
                <Icon className="w-6 h-6" />
              </div>
              <div>
                <p className="text-sm text-[#6B6B6B]">{stat.label}</p>
                <p className="text-2xl font-bold text-[#1C1C1C] mt-1">{stat.value}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="bg-white p-6 rounded-3xl border border-[#F5C6D0]/30 shadow-sm">
        <h2 className="text-lg font-bold text-[#1C1C1C] mb-4">Recent Activity</h2>
